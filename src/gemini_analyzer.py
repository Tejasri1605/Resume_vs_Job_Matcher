...
def get_valid_model(preferred_model="gemini-2.5-flash"):
    """
    Returns a valid Gemini model object.
    Picks a known working model by default.
    """
    try:
        logger.info(f"Using Gemini model: {preferred_model}")
        return genai.GenerativeModel(preferred_model)
    except Exception as e:
        raise GeminiError(f"Failed to retrieve a valid Gemini model: {e}") from e
...
[A single number between 0 and 100]
    ...
    try:
        ...
        analysis_result["resume_summary"] = summary_match.group(1).strip() if summary_match else "Could not parse Resume Summary."

        jd_match = re.search(r"JD_SUMMARY:(.*?)REQUIREMENTS_MET:", text, re.DOTALL | re.IGNORECASE)
        analysis_result["jd_summary"] = jd_match.group(1).strip() if jd_match else "Could not parse Job Description Summary."

        met_match = re.search(r"REQUIREMENTS_MET:(.*?)REQUIREMENTS_MISSING:", text, re.DOTALL | re.IGNORECASE)
        analysis_result["matches"] = met_match.group(1).strip() if met_match else "Could not parse Requirements Met."

        missing_match = re.search(r"REQUIREMENTS_MISSING:(.*?)PERCENTAGE_MATCH:", text, re.DOTALL | re.IGNORECASE)
        analysis_result["misses"] = missing_match.group(1).strip() if missing_match else "Could not parse Requirements Missing."
        ...
        if not 0 <= analysis_result["percentage"] <= 100:
            logger.warning(f"Parsed percentage ({analysis_result['percentage']}) out of range. Setting to 0.")
